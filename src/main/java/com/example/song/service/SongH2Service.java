...
public class SongH2Service implements SongRepository {
    ...
    public Song addSong(Song song) {
        ...
        Song savedSong = db.queryForObject("select * from playlist where songName = ? and lyricist = ? and singer = ?",
                new SongRowMapper(),
    ...
    public Song updateSong(int songId, Song song) {
        if (song.getSongName() != null) {
            db.update("update playlist set songName = ? where songId = ?", song.getSongName(), song.getSongId());
        }
        if (song.getLyricist() != null) {
            db.update("update playlist set lyricist = ? where songId = ?", song.getLyricist(), song.getSongId());
        }
        if (song.getSinger() != null) {
            db.update("update playlist set singer = ? where songId = ?", song.getSinger(), song.getSongId());
        }
